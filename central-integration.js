...
(()=>{'use strict';
 ...
 function applyRutinWorkToApp(state){const raw=sessionStorage.getItem('HANE_LAST_RUTIN_IMPORT');if(!raw)throw new Error('Aktarım oturumu bulunamadı');const obj=JSON.parse(raw),s=obj.state||{};state.work=Array.isArray(state.work)?state.work:[];state.expenses=Array.isArray(state.expenses)?state.expenses:[];let workAdded=0,workSkipped=0,roadAdded=0,roadSkipped=0;
   ...
   sessionStorage.removeItem('HANE_LAST_RUTIN_IMPORT');return {workAdded,workSkipped,roadAdded,roadSkipped}
 }
