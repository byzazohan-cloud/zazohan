/* HANE V0.4 - RUTIN backup -> central + visible work migration */
(()=>{'use strict';
 async function importRutinFile(file){const text=await file.text();let obj;try{obj=JSON.parse(text)}catch(_){throw new Error('RUTİN yedeği JSON olarak okunamadı')};if(window.HANE_CORE.detectBackup(obj)!=='RUTIN-BACKUP-V3')throw new Error('Desteklenmeyen RUTİN yedek biçimi');const result=window.HANE_CORE.importRutin(obj,file.name||'RUTIN yedeği');window.__HANE_LAST_RUTIN_IMPORT__=obj;return result}
 const key=x=>String(x?._sourceId||x?.id||'');
 function applyRutinWorkToApp(state){const obj=window.__HANE_LAST_RUTIN_IMPORT__;if(!obj)throw new Error('Aktarım oturumu bulunamadı');const s=obj.state||{};state.work=Array.isArray(state.work)?state.work:[];state.expenses=Array.isArray(state.expenses)?state.expenses:[];let workAdded=0,workSkipped=0,roadAdded=0,roadSkipped=0;
   ...
   try{delete window.__HANE_LAST_RUTIN_IMPORT__}catch(_){window.__HANE_LAST_RUTIN_IMPORT__=null}return {workAdded,workSkipped,roadAdded,roadSkipped}
 }
 window.HANE_IMPORT=Object.freeze({importRutinFile,applyRutinWorkToApp,summary:()=>window.HANE_CORE.summary()});
})();
