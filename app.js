...
const state={
  ...
  storagePersistent:null,audioToken:0
};
...
const providerLabel=i=>i?.provider==='youtube'?'YouTube':i?.provider==='apple'?'Apple Önizleme':i?.provider==='jamendo'?'Jamendo':i?.source||'Internet Archive';
const providerClass=i=>i?.provider==='youtube'?'youtube':i?.provider==='apple'?'apple':i?.provider==='jamendo'?'jamendo':'archive';
const sourcePill=i=>`<span class="source-pill ${providerClass(i)}">${esc(providerLabel(i))}${i?.preview?' · 30 sn':''}</span>`;
...
function sourceSummary(type){const parts=['Internet Archive'];if(type==='music')parts.push('Apple Önizleme');if(state.jamendoClientId&&type==='music')parts.push('Jamendo');if(state.youtubeKey)parts.push('YouTube');return parts.join(' · ')}
function searchHeader(type){
  if(type==='music')return `<section class="music-search-hero"><span class="eyebrow">ZAZO AKILLI ARAMA</span><h2>Ne dinlemek istiyorsun?</h2><p>Şarkı adı, sanatçı veya albüm yaz. Kaynak seçmen gerekmez.</p><div class="searchbar music-main-search"><span class="search-icon">⌕</span><input id="searchInput" value="${esc(state.musicQuery)}" placeholder="Şarkı veya sanatçı ara…" enterkeyhint="search" autocomplete="off"><button id="searchBtn">Ara</button></div></section>`;
...
function searchAvailabilityBadge(i){if(i.preview)return '<span class="availability preview">30 sn önizleme</span>';if(i.identifier&&!i.verifiedPlayable)return '<span class="availability checking">◌ Oynatma kontrol edilir</span>';if(i.provider==='youtube')return '<span class="availability youtube">▶ YouTube</span>';return '<span class="availability full">● Tam parça</span>'}
function musicSearchRow(i,queue=null){const id=registerUi(i,'music',queue);return `<button class="song-result" onclick="openUi('${id}')"><span class="song-art">${i.thumb?`<img loading="lazy" decoding="async" src="${esc(i.thumb)}" alt="">`:'♫'}</span><span class="song-info"><strong>${esc(i.title)}</strong><small>${esc(i.creator||'Bilinmeyen sanatçı')}</small>${searchAvailabilityBadge(i)}</span><span class="song-play">▶</span></button>`}
function renderMusic(){
  resetUiRegistry();
  const chips=['Pop','Türkçe Pop','Rock','Rap','Arabesk','Caz'],results=state.results.music,disc=state.discovery.music;
  const resultsHtml=state.musicSearching?`<section class="section"><div class="searching-card"><span class="search-spinner">◌</span><strong>Şarkı aranıyor…</strong><small>${state.lowPower?'Düşük güçte önce doğrudan ses kaynakları taranıyor.':'Bağlı kataloglar taranıyor.'}</small></div></section>`:results.length?`<section class="section search-results-section"><div class="section-head"><div><h2>Bulunan şarkılar</h2><div class="muted">${results.length} sonuç · doğrudan ses kaynakları öncelikli</div></div><button onclick="clearResults('music')">Temizle</button></div><div class="song-results">${results.map(x=>musicSearchRow(x,results)).join('')}</div></section>`:state.musicQuery?`<section class="section"><div class="empty">${state.searchError.music?esc(state.searchError.music):`“${esc(state.musicQuery)}” için oynatılabilir sonuç bulunamadı.<br><small>Sanatçı adıyla veya daha kısa bir aramayla tekrar deneyebilirsin.</small>`}</div></section>`:`<div class="chips music-chips">${chips.map(c=>`<button class="chip" data-q="${c}">${c}</button>`).join('')}</div>${state.recentSearches.length?`<section class="recent-searches"><div class="section-head compact"><h3>Son aramalar</h3><button onclick="clearRecentSearches()">Temizle</button></div><div class="recent-chips">${state.recentSearches.map(q=>`<button class="recent-search-btn" data-recent="${esc(q)}">⌕ ${esc(q)}</button>`).join('')}</div></section>`:''}<section class="music-feature"><div><span>TEK ARAMA</span><h2>Şarkıyı yaz, gerisini ZAZO yapsın.</h2><p>Tam oynatılabilen sonuçları önceliklendirir; yalnızca önizleme varsa bunu açıkça belirtir.</p></div><button onclick="document.getElementById('searchInput')?.focus()">⌕</button></section><section class="section"><div class="section-head"><h2>Keşfet</h2><span class="muted">Hemen dinle</span></div><div id="musicDiscovery">${disc.length?`<div class="h-scroll wide">${disc.slice(0,10).map(x=>mediaCard(x,'music',disc)).join('')}</div>`:'<div class="empty">İçerikler hazırlanıyor…</div>'}</div></section>`;
...
async function appleSearch(q,limit=10,signal=null){const url=`https://itunes.apple.com/search?term=${encodeURIComponent(q)}&media=music&entity=song&country=TR&limit=${limit}`;const d=await jsonp(url,{signal,timeout:7500});return (d.results||[]).filter(x=>x.previewUrl).map(x=>({id:`apple:${x.trackId}`,title:x.trackName||'Parça',creator:x.artistName||'Apple Katalog',source:'Apple katalog önizlemesi',provider:'apple',preview:true,type:'music',url:String(x.previewUrl).replace(/^http:/,'https:'),thumb:String(x.artworkUrl100||'').replace('100x100','600x600'),album:x.collectionName||'',duration:Math.round((x.trackTimeMillis||0)/1000)}))}
async function jamendoSearch(q,limit=10,signal=null){if(!state.jamendoClientId)return[];const u=`https://api.jamendo.com/v3.0/tracks/?client_id=${encodeURIComponent(state.jamendoClientId)}&format=json&limit=${limit}&search=${encodeURIComponent(q)}&audioformat=mp32&imagesize=300`;const r=await fetchWithTimeout(u,{signal,timeout:8000});const d=await r.json();if(!r.ok||d.headers?.status==='failed')throw Error('jamendo');return (d.results||[]).filter(x=>x.audio).map(x=>({id:`jamendo:${x.id}`,title:x.name||'Parça',creator:x.artist_name||'Jamendo',source:'Jamendo',provider:'jamendo',type:'music',url:x.audio,thumb:x.image||x.album_image||'',shareUrl:x.shareurl||'',duration:Number(x.duration)||0,license:x.license_ccurl||'',verifiedPlayable:true}))}
...
function musicProviderRank(i){if(i.provider==='jamendo')return 70;if(i.provider==='archive'||i.identifier)return i.verifiedPlayable?62:36;if(i.provider==='youtube')return 52;if(i.preview||i.provider==='apple')return 18;return 25}
function relevanceScore(i,q){const query=normalizeWords(q),title=normalizeWords(i.title),creator=normalizeWords(i.creator);if(!query)return 0;const toks=query.split(' ').filter(Boolean);let score=0;if(title===query)score+=80;if(`${creator} ${title}`===query||`${title} ${creator}`===query)score+=100;if(title.includes(query))score+=40;if(creator.includes(query))score+=22;for(const t of toks){if(title.includes(t))score+=9;if(creator.includes(t))score+=6;}return score}
...
async function runSearchJobs(type,query,signal){
  ...
  if(type==='music'){
    const base=[iaSearch(type,query,small?8:14,signal),appleSearch(query,small?8:18,signal)];if(state.jamendoClientId)base.push(jamendoSearch(query,small?8:14,signal));
    await settle(base);
    const hasDirect=results.some(x=>(x.provider==='jamendo'&&relevanceScore(x,query)>=24)||(x.provider==='archive'&&x.verifiedPlayable&&relevanceScore(x,query)>=24));
    if(state.youtubeKey&&(!small||!hasDirect))await settle([youtubeSearch(type,query,small?8:18,signal)]);
  }else{
...
async function openItem(i,t,preserveQueue=false,sourceQueue=null){
  ...
  if(!preserveQueue){const src=(Array.isArray(sourceQueue)&&sourceQueue.length?sourceQueue:[i]).map(hydrate);const idx=src.findIndex(x=>key(x)===key(i));state.queue=src;state.queueIndex=idx>=0?idx:0;}
  play(i,t);
}
...
function drawLibrary(mode){
  ...
  if(mode==='files')b.innerHTML=state.localMedia.length?`<div class="list">${listRows(state.localMedia)}</div>`:'<div class="empty">Kendi MP3 veya videonu ekleyebilirsin.</div>';
...
function renderSettings(){
  ...
  view.innerHTML=`<div class="section-head"><h2>Ayarlar</h2></div><div class="setting"><div><strong>Düşük Güç Modu</strong><div class="muted">Animasyon, sonuç sayısı ve gereksiz ağ istekleri azaltılır</div></div><button id="lpSwitch" class="switch ${state.lowPower?'on':''}"></button></div><section class="connector-box"><div class="connector-head"><div><strong>İnternet Araması</strong><div class="muted">Kaynaklar arka planda otomatik kullanılır</div></div><span class="source-pill archive">Akıllı</span></div><div class="connector always"><div><strong>Internet Archive</strong><small>Açık/izinli tam içerikler</small></div><span>✓ Aktif</span></div><div class="connector always"><div><strong>Apple kataloğu</strong><small>Şarkı eşleştirme ve yaklaşık 30 sn önizleme</small></div><span>✓ Aktif</span></div><div class="connector always"><div><strong>YouTube</strong><small>Resmî gömülü oynatıcı</small></div><span>${state.youtubeKey?'✓ Bağlı':'— Yapılandırılmadı'}</span></div><div class="connector always"><div><strong>Jamendo</strong><small>İzinli bağımsız müzik akışları</small></div><span>${state.jamendoClientId?'✓ Bağlı':'— Yapılandırılmadı'}</span></div></section><div class="setting"><div><strong>Çevrimdışı depolama</strong><div class="muted">${state.downloads.length} indirme · ${formatBytes(total)}</div></div><button id="clearDownloadsBtn" class="text-btn danger-text" ${state.downloads.length?'':'disabled'}>Temizle</button></div><div class="setting"><div><strong>Depolama koruması</strong><div class="muted">Çevrimdışı dosyaların sistem tarafından temizlenme riskini azaltır</div></div><span>${state.storagePersistent===true?'✓ Kalıcı':state.storagePersistent===false?'Sistem yönetiyor':'Kontrol ediliyor'}</span></div><div class="setting"><div><strong>Kaldığın yerden devam</strong><div class="muted">Müzik ve video konumu otomatik kaydedilir</div></div><span>✓</span></div><div class="setting"><div><strong>Kilit ekranı kontrolleri</strong><div class="muted">Yerel ve doğrudan akışlarda Media Session</div></div><span>✓</span></div><div class="setting"><div><strong>Sürüm</strong><div class="muted">ZAZO PLAYER V1.2.9 STABILITY</div></div><span>1.2.9</span></div>`;
...
(async()=>{await refreshStored();render();if('serviceWorker'in navigator)navigator.serviceWorker.register('sw.js').catch(()=>{});try{if(navigator.storage?.persisted)state.storagePersistent=await navigator.storage.persisted();if(state.storagePersistent!==true&&navigator.storage?.persist)state.storagePersistent=await navigator.storage.persist();}catch{state.storagePersistent=false}if(state.tab==='settings')renderSettings();})();
